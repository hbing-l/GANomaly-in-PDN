...
class Encoder(nn.Module):
    ...
    def __init__(self, imageSize, nz, nc, ndf, ngpu, n_extra_layers=0, add_final_conv=True):
        ...
        self.ngpu = ngpu
        assert imageSize % 12 == 0, "imageSize has to be a multiple of 16"
...
class Decoder(nn.Module):
    ...
    def __init__(self, imageSize, nz, nc, ngf, ngpu, n_extra_layers=0):
        ...
        # cngf, timageSize = ngf // 2, 4   # //取整除，向下取整
        ...
        main.add_module('final-{0}-{1}-convt'.format(cngf, nc),
                        nn.ConvTranspose2d(cngf, nc, 2, 2, 2, bias=False))
...
class NetD(nn.Module):
    ...
    def forward(self, x):
        ...
        f = features.reshape(-1,32*3*3)
        classifier = self.add_layer(f).squeeze(1)
...
def print_net():
    ...
    opt = OPT(12, 100, 1, 16, 1, 0)
    gen = NetG(opt).to(device)
    opt = OPT(12, 1, 1, 16, 1, 0)
    disc = NetD(opt).to(device)
    summary(gen, (1, 12, 12))
    summary(disc, (1, 12, 12))
